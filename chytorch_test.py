...
class ReactionCenterPredictor(nn.Module):
    def __init__(
        ...
        self.molecule_encoder = MoleculeEncoder(
            d_model=hidden_size, max_tokens=max_tokens
        )
        self.rule_embedding = nn.Embedding(
            num_rules, hidden_size * max_tokens
        )  # Embedding to a larger space
        ...
        self.output = nn.Linear(
            hidden_size, 1
        )  # Output layer mapping to 1 for each atom

    def forward(self, molecules: torch.Tensor, rule_idx: torch.Tensor) -> torch.Tensor:
        mol_encoding = self.molecule_encoder(
            molecules
        )  # Shape: (batch, seq_len, hidden_size)

        # Embed rule and reshape to (batch, max_tokens, hidden_size)
        rule_emb = self.rule_embedding(
            rule_idx
        )  # Shape: (batch, max_tokens * hidden_size)
        print("rule_emb shape", rule_emb.shape)
        rule_emb = rearrange(
            rule_emb, "b (t h) -> t b h", t=self.max_tokens, h=self.hidden_size
        )
        print("rule_emb shape after rearrange", rule_emb.shape)
        # Cross-attention
        mol_repr = rearrange(
            mol_encoding, "b t h -> t b h"
        )  # Shape: (seq_len, batch, hidden_size)
        attn_output, _ = self.attention(rule_emb, mol_repr, mol_repr)

        # Combine and project
        combined = rearrange(attn_output, "t b h -> b t h")  # Keep sequence dimension
        x = self.fc(combined)  # Apply fully connected layer to each time step
        x = F.gelu(x)  # Apply GeLU activation
        print("x shape before output", x.shape)
        x = self.output(x).squeeze(
            -1
        )  # Output layer, should map to 1 for each atom in the sequence

        # Pad the output to match max_tokens
        if x.size(1) < self.max_tokens:
            padding_size = self.max_tokens - x.size(1)
            x = F.pad(x, (0, padding_size), mode="constant", value=0)
        print("x shape after output and pading", x.shape)
...
class ReactionDataset(Dataset):
    def __init__(
        ...
        self.smiles_dataset = SMILESDataset(
            [encode(smi) for smi in meta_data["mapped_reactants"].tolist()], cache={}
        )
        ...
        for idx, reactant in enumerate(meta_data["mapped_reactants"]):
        ...
        for reactant, indices in self.reactant_indices.items():
            for rule_idx in range(self.num_rules):
                rule = self.rules_dict[
                    rule_idx + 1
                ]  # +1 because rules_dict is 1-indexed
                # reactant_smiles = meta_data['Reactants'].iloc[indices[0]]
                reactant_smiles = meta_data["mapped_reactants"].iloc[indices[0]]
        ...
        self.weights = [weight_applicable] * num_applicable + [
            weight_non_applicable
        ] * num_non_applicable
...
def create_train_val_test_split(
    dataset: ReactionDataset, train_ratio: float = 0.8, test_ratio: float = 0.1
):
...
def train_model(
    ...
    model.train()
    total_loss = 0.0
    for reactants, rule_indices, targets in tqdm(
        train_loader, desc=f"Epoch {epoch+1} (Train)", leave=False, position=0
    ):
...
def find_optimal_threshold(
    ...
    if optimization_metric == "youden":
        ...
        raise ValueError(
            "Invalid optimization metric. Choose 'youden', 'distance', or 'f1'."
        )
...
def validate_model(
    ...
    optimal_threshold, roc_auc, fpr, tpr, thresholds = find_optimal_threshold(
        all_targets.ravel(), all_outputs.ravel()
    )
...
def main():
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=batch_size, sampler=val_sampler, collate_fn=collate_fn
    )
    ...
    print("device", device)
    model = ReactionCenterPredictor(
        num_rules=len(rules_dict), hidden_size=hidden_size, max_tokens=max_tokens
    ).to(device)
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Epochs"):
        ...
        val_loss, val_accuracy, optimal_threshold = validate_model(
            model, val_loader, criterion, device
        )

        print(
            f"Epoch {epoch+1}/{num_epochs}, Train Average Loss: {train_loss:.4f}, Validation Accuracy: {val_accuracy:.4f}"
        )
    ...
    applicability_accuracy = evaluate_applicability(
